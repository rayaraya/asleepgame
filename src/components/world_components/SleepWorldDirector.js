...
class SleepWorldDirector extends WorldDirector {
    ...
    constructNewWorld() {
        ...
        let beginning = this.evBuilder.buildUnit({
            name : "beginning",
        ...
        let storm = this.evBuilder.buildUnit({
            name : "storm",
            frequency : 10, 
        ...
        let chapel = this.loBuilder.buildUnit({
            name : "The Chapel",
            world : this.world,
            view : "here will be class with control buttons"
        })

        let player = this.chBuilder.buildUnit({
            name : "Player",
            location : chapel
        })

        let oldMan = this.chBuilder.buildUnit({
            name : "Old Man",
            location : chapel
        })
